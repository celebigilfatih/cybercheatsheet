...
async function inspectAircrack() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Aircrack-ng Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found Aircrack-ng Cheatsheet:');
            ...
            console.log('Aircrack-ng Cheatsheet NOT FOUND');
        }
