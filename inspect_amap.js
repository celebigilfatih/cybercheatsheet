...
async function inspectAmap() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Amap Cheat Sheet' }).lean();

        if (sheet) {
            console.log('Found Amap Cheatsheet:');
            ...
            console.log('Amap Cheatsheet NOT FOUND');
        }
