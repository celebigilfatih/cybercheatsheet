...
async function inspectBinwalk() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'Binwalk Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('Binwalk not found');
            return;
        }
        console.log('Found Binwalk Cheatsheet:');
