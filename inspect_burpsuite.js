...
async function inspectBurp() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Burp Suite Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found Burp Suite Cheatsheet:');
            ...
            console.log('Burp Suite Cheatsheet NOT FOUND');
        }
