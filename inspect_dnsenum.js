...
async function inspectDnsenum() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'dnsenum Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found dnsenum Cheatsheet:');
            ...
            console.log('dnsenum Cheatsheet NOT FOUND');
        }
