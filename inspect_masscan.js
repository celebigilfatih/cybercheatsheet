...
async function inspectMasscan() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Masscan Cheat Sheet' }).lean();

        if (sheet) {
            console.log('Found Masscan Cheatsheet:');
            ...
            console.log('Masscan Cheatsheet NOT FOUND');
        }
