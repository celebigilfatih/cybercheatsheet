...
async function inspectMedusa() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'Medusa Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('Medusa not found');
            return;
        }
        console.log('Found Medusa Cheatsheet:');
