...
async function inspectMetasploit() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Metasploit Framework' }).lean();

        if (sheet) {
            console.log('Found Metasploit Cheatsheet:');
            ...
            console.log('Metasploit Cheatsheet NOT FOUND');
        }
