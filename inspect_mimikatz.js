...
async function inspectMimikatz() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'Mimikatz Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('Mimikatz not found');
            return;
        }
        console.log('Found Mimikatz Cheatsheet:');
