...
async function inspectNcrack() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Ncrack Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found Ncrack Cheatsheet:');
            ...
            console.log('Ncrack Cheatsheet NOT FOUND');
        }
