...
async function inspectNetcat() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Netcat Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found Netcat Cheatsheet:');
            ...
            console.log('Netcat Cheatsheet NOT FOUND');
        }
