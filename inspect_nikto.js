...
async function inspectNikto() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'Nikto Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('Nikto not found');
            return;
        }
        console.log('Found Nikto Cheatsheet:');
