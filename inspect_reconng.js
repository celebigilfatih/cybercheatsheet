...
async function inspectReconNg() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'recon-ng Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found recon-ng Cheatsheet:');
            ...
            console.log('recon-ng Cheatsheet NOT FOUND');
        }
