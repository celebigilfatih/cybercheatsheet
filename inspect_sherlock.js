...
async function inspectSherlock() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'Sherlock Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('Sherlock not found');
            return;
        }
        console.log('Found Sherlock Cheatsheet:');
