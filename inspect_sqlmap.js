...
async function inspectSqlmap() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'SQLMap Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('SQLMap not found');
            return;
        }
        console.log('Found SQLMap Cheatsheet:');
