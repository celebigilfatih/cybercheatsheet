...
async function inspectTshark() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'TShark Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found TShark Cheatsheet:');
            ...
            console.log('TShark Cheatsheet NOT FOUND');
        }
