...
async function inspectWhatWeb() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'WhatWeb Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found WhatWeb Cheatsheet:');
            ...
            console.log('WhatWeb Cheatsheet NOT FOUND');
        }
