...
async function inspectWifite() {
    try {
        await dbConnect();
        const cs = await Cheatsheet.findOne({ 'title.en': 'Wifite Cheat Sheet' }).populate('category');
        if (!cs) {
            console.log('Wifite not found');
            return;
        }
        console.log('Found Wifite Cheatsheet:');
