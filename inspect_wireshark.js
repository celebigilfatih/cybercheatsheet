...
async function inspectWireshark() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'Wireshark Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found Wireshark Cheatsheet:');
            ...
            console.log('Wireshark Cheatsheet NOT FOUND');
        }
