...
async function inspectZap() {
    try {
        await dbConnect();
        const sheet = await Cheatsheet.findOne({ 'title.en': 'OWASP ZAP Cheat Sheet' }).populate('category').lean();

        if (sheet) {
            console.log('Found OWASP ZAP Cheatsheet:');
            ...
            console.log('OWASP ZAP Cheatsheet NOT FOUND');
        }
