    ...
    <main className="mx-auto max-w-5xl p-6 space-y-4">
      ...
      <div className="panel p-4">
        ...
        <ul className="space-y-2">
          {categories.map((c) => {
            const displayName = c.name?.[language] || c.name?.tr || c.name || ''
            const displayDesc = c.description?.[language] || c.description?.tr || c.description || ''
            return (
              <li key={c._id} className="flex items-center gap-2">
                <input defaultValue={displayName} onBlur={(e) => update(c._id, { name: e.target.value, description: c.description })} className="px-2 py-1 rounded bg-gray-900 border border-gray-800" />
                <input defaultValue={displayDesc} onBlur={(e) => update(c._id, { name: c.name, description: e.target.value })} className="flex-1 px-2 py-1 rounded bg-gray-900 border border-gray-800" />
